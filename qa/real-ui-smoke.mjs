...
let page='sos',currentSession=null,stored={profile:null,contacts:[],events:[],deliveries:[]},created=0,requests=[],notices=[],healthReady=false,resent=[];
...
const cloud={client:{auth:{onAuthStateChange(){}}},session:async()=>currentSession,
 ...
 api:async b=>{requests.push(b);if(b.action==='health')return {ready:healthReady,enabled:healthReady,authorizedTest:true,authorizedUser:true,missing:[],worker:true};
...
page='profile';render();fill('#real-name','Persona ficticia');fill('#real-phone','+12025550101');await submit('#real-profile-form');assert.equal(stored.profile.name,'Persona ficticia');
page='contacts';render();click('[data-real-action="add-contact"]');fill('#rc-name','Contacto ficticio');fill('#rc-phone','123');fill('#rc-relation','Familiar');await submit('#real-contact-form');assert.match(q('#real-contact-form .form-error').textContent,/código de país/);fill('#rc-phone','+1 202 555 0100');q('[name=consent]').checked=true;await submit('#real-contact-form');assert.equal(stored.contacts.length,1);assert.equal(stored.contacts[0].phone,'+12025550100');
...
healthReady=true;page='sos';render();click('[data-real-action="refresh"]');await settle();assert.equal(q('[data-real-action="start-sos"]').disabled,false);
...
click('[data-real-action="start-sos"]');await settle();assert.equal(created,2);stored.events[0].status='processed';stored.deliveries=[{event_id:stored.events[0].id,contact_name:'Contacto editado',phone:'+12025550100',status:'accepted'}];await intervals[0]();await settle();assert.match(q('main').textContent,/Procesamiento finalizado/);
