import {createInternalUI} from './internal-ui.js';
...
export function createRealUI({onChange,onDemo,notify,loadCloud=()=>import('./cloud.js').then(m=>m.connectCloud())}){
 const internal=createInternalUI({onChange,notify});
 let sosChannel='internal';
 let cloud,initialized=false,loading=false,session=null,health=null,model={profile:null,contacts:[],events:[],deliveries:[]};
 let error='',healthError='',busy=false,currentPage='sos',editor=null,profileEdit=false,detail=null,recovery=null,epoch=0,clockOffset=0,polling=false;
 ...
 function reset(){internal.reset();epoch++;session=null;health=null;healthError='';model={profile:null,contacts:[],events:[],deliveries:[]};editor=null;detail=null;profileEdit=false;recovery=null;dialog.close();}
 async function refresh(all=true){
   ...
   const before=JSON.stringify(model);model=data;await internal.attach(cloud,session,model.profile);
   if(all){
     try{const nextHealth=await cloud.api({action:'health'});if(version!==epoch)return;health=nextHealth;healthError='';}
     catch{if(version!==epoch)return;health=null;healthError='Tu sesión está activa y puedes guardar contactos y perfil. No se pudo consultar el servicio SOS. Pulsa «Actualizar estado» para reintentar.';}
   }
 ...
 function more(){return `${heading('Tu espacio personal','Gestiona tu cuenta y revisa los envíos.')}<div class="more-grid">${[['history','history','Central / Historial','Bandeja de alertas y resultados'],['contacts','users','Contactos','Tu círculo de confianza'],['profile','user','Perfil','Tu nombre para las alertas'],['security','lock','Seguridad','PINs y Tutor · solo demostración']].map(([p,i,t,d])=>`<button class="card more-card" data-page="${p}"><span class="icon-tile">${icon(i)}</span><span><strong>${t}</strong><small>${d}</small></span>${icon('chevron')}</button>`).join('')}</div>`;}
 ...
 async function action(action,el){
   if(action==='channel-internal'||action==='channel-whatsapp'){sosChannel=action==='channel-internal'?'internal':'whatsapp';changed();return;}
   ...
   try{
     if(action==='logout'){if(internal.hasActive()||model.events.some(e=>['pending','dispatching'].includes(e.status)))throw Error('Hay un SOS pendiente. Cancélalo en SOS o espera su resultado antes de cerrar sesión.');await cloud.signOut();reset();}
 ...
 return {view,init,hasActive:()=>internal.hasActive()||model.events.some(e=>['pending','dispatching'].includes(e.status)),accountName:()=>model.profile?.name||'Tu cuenta'};
}
